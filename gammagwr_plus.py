...
class GammaGWR:
    ...
    def train(self, dataSet, labelSet, maxEpochs, insertionT, beta, epsilon_b, epsilon_n, context):
        ...
        for epoch in range(0, self.maxEpochs):
            for iteration in range(0, self.samples):
                ...
                if ((not self.regularized) and (a < self.insertionThreshold) and (h < self.habThreshold) and (self.numNodes < self.maxNodes)) or ((self.regularized) and (label!=winnerLabelIndex) and (a < self.insertionThreshold) and (h < self.habThreshold) and (self.numNodes < self.maxNodes)):
                    # Add new weight
                    ...
                    if ((self.regularized) and (label==winnerLabelIndex) and (h < self.habThreshold)):
                        updateRate_b = self.epsilon_b * 0.01
                        updateRate_n = self.epsilon_n * 0.01
                    else:
